...
export class Ingredient extends Component {
  constructor(props) {
    ...
    this.rotate = window.matchMedia('(orientation: landscape)').matches
      ? this.vertical.includes(this.props.type)
        ? 90
        : this.verticalCounter.includes(this.props.type)
        ? -90
        : 0
      : 0;
    this.state = {
      ...
      cursor: 'grab',
      ingred:
        this.ingred.length === undefined ? this.ingred : this.ingred[Math.floor(Math.random() * this.ingred.length)],
    };
  }
  ...
  itemZChanger = (direction) => {
    ...
    this.props.ingreds.forEach((el, idx) => {
    ...
    switch (mode) {
      case 'rotate':
        this.setState((prevState) => ({
          rotate: prevState.rotate + value,
          touchInitialRotate: prevState.touchInitialRotate + value,
        }));
        break;
      case 'counter':
        this.setState((prevState) => ({
      ...
      case 'enlarge':
        if (this.state.scale < 6) {
          this.setState((prevState) => ({
            scale: prevState.scale + value / 100,
            touchInitialScale: prevState.touchInitialScale + value / 100,
          }));
        }
        break;
      case 'shrink':
  ...
  onMouseDownHandler = (e) => {
    ...
    this.setState((prevState) => ({
      relX: clientX - prevState.x,
      relY: clientY - prevState.y,
  ...
  onRotateStartHandler = (e) => {
    ...
    this.setState((prevState) => ({
      touchInitialRotate: prevState.touchInitialRotate - Math.round(e.rotation),
      touchRotate: true,
    }));
  };
